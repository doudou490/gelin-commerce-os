...
app.get('/auth/google/callback',(req,res)=>res.status(501).send('Google OAuth callback needs persistent user/session storage before accounts can be created.'));
app.get('/auth/facebook/callback',(req,res)=>res.status(501).send('Facebook OAuth callback needs persistent user/session storage before accounts can be created.'));
